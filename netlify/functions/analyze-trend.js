...
// Prompt cho phân tích chi tiết (Analyze)
function createDetailedAnalysisPrompt(trend, language) {
    ...
    if (language === 'vi') {
        ...
        return `
            You are a marketing trend analyst. Analyze the following news item.
...
// Prompt mới cho phân tích dự đoán (Predict)
...
exports.handler = async (event) => {
    ...
    if (event.httpMethod === "GET") {
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: "AI service is online." }) };
    }
    ...
    try {
        ...
        if (analysisType === 'summary') {
            // Logic summary cũ (được dùng bởi nút Analyze, không phải nút Predict)
