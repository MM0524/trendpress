...
// Generic RSS parser
function parseRssItem(block, defaults = {}) {
  const title = (block.match(/<title><!\[CDATA\[(.*?)\]\]><\/title>/) ||
    block.match(/<title>(.*?)<\/title>/) || [])[1] || defaults.title;
...
async function fetchHackerNewsFrontPage() {
  try {
    ...
    while ((match = itemRegex.exec(xml)) && items.length < 25) {
      const { title, description, link, pubDate } = parseRssItem(match[1], { title: 'Hacker News' });
...
async function fetchGoogleNewsTop() {
  try {
    ...
    while ((match = itemRegex.exec(xml)) && items.length < 25) {
      const { title, description, link, pubDate } = parseRssItem(match[1], { title: 'Google News' });
...
async function fetchVnExpressInternational() {
  try {
    ...
    while ((match = itemRegex.exec(xml)) && items.length < 25) {
      const { title, description, link, pubDate } = parseRssItem(match[1], { title: 'VnExpress News' });
...
async function fetchTikTokTrends() {
  ...
  try {
    ...
    return list.slice(0, 15).map((v) => ({
      ...
      description: stripHtml(v.desc || 'Trending on TikTok'),
      category: 'TikTok',
      ...
      source: v.webVideoUrl || v.shareUrl ||
        `https://www.tiktok.com/@${v?.author?.uniqueId || ''}/video/${v?.id || v?.video_id || ''}`,
...
async function fetchInstagramTrends() {
  ...
  try {
    ...
    return list.slice(0, 15).map((p) => ({
      ...
      description: stripHtml(p.caption || 'Trending on Instagram'),
      category: 'Instagram',
      tags: (p.hashtags || [])
